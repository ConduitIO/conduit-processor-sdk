...
func testRename(t *testing.T, resolver Resolver, newReference string, rec opencdc.Record, want any, wantErr bool) {
	...
	is.NoErr(err)
	// create a resolver for the new field name
	resolver, err = NewResolver(newReference)
	is.NoErr(err)
	ref, err = resolver.Resolve(&rec)
	is.NoErr(err)
	is.Equal(ref.Get(), want)
}
